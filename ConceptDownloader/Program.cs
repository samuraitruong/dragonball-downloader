    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            if (!String.IsNullOrEmpty(options.Url) && options.CrawlMode)
            {
                ...
                var urls = SimpleCrawler.GetLinks(options.Url).Result;
                foreach (var item in urls)
                {
                    if (!string.IsNullOrEmpty(options.Filter) && !item.Name.ToLower().Contains(options.Filter.ToLower())) continue;
                    Console.Clear();
                   // Console.WriteLine(item.Url);
                    DownloadFileWithMultipleThread(item.Url, options.Output, options.Thread, options.Buffer);
                }
